...
echo "DELETE FROM request_group WHERE NOT EXISTS (SELECT request FROM request WHERE request.request_group = request_group.request_group)" | su - www-data psql shotserver03
